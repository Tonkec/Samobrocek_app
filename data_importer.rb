class DataImporter
  def initialize(opts)
    @page_schema = opts[:page_schema] ||
      raise(ArgumentError.new("can't find
        :page_schema in the arguments"))
  ...
  def import_all
    ...
    @page_schema[:departure_positions].each do |day_type, position|
  ...
  private
    ...
    def import_line
      line_name = 
        @page.get_title(@page_schema[:line_name]).first
      Line.create(title: line_name)
    end

    def import_day_types
      day_types  = @page.get_title(@page_schema[:day_types])
      day_types.each {|t| DayType.create(title: t)}
    end

    def import_directions
      directions = @page.get_title(@page_schema[:directions])
      directions.each {|t| Direction.create(title: t)}
    end

    def import_route_types
      types      = @page.get_title(@page_schema[:route_types])
